...
import { sendAiRequest } from '../src/services/aiClient';
import { mapFrontendResponsesToAi, formatAiBaselineToAssessmentResult } from '../src/adapters/assessmentAdapter';
import { mapAiActivityToGame } from '../src/adapters/gameAdapter';
...
async function runTests() {
  console.log('\n=============================================================');
  console.log('🧪 RUNNING SMRITI (TEAM AXIOM) END-TO-END INTEGRATION TEST SUITE');
  console.log('=============================================================\n');
  ...
  try {
    if (!BASE_URL) {
      ...
      console.log(`📡 Spawned ephemeral test server at: ${BASE_URL}\n`);
    }
    ...
    if (asstRes.status === 200 && asstRes.data.success && asstRes.data.response) {
