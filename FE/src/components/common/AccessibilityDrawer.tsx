...
import { Type, Eye, Volume2, Gauge, Smartphone } from 'lucide-react';
...
export const AccessibilityDrawer: React.FC<AccessibilityDrawerProps> = ({
  ...
  const { accessibility, updateAccessibility, t, speakText, isSimpleElderlyMode, toggleSimpleElderlyMode } = useApp();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('accessibility.title')} maxWidth="lg">
      <div className="space-y-6 text-slate-800">
        ...
        {/* Simple Elderly Mode Big Highlight Toggle */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${
                isSimpleElderlyMode ? 'bg-amber-500 text-slate-950' : 'bg-slate-200 text-slate-700'
              }`}>
                <Smartphone size={22} />
              </div>
              <div>
                <div className="font-bold text-slate-900 text-base flex items-center gap-2">
                  {t('accessibility.simpleMode')}
                  {isSimpleElderlyMode && (
                ...
                <div className="text-xs text-slate-600">{t('accessibility.simpleModeDesc')}</div>
              </div>
            </div>
            <button
              onClick={() => {
                toggleSimpleElderlyMode();
                speakText(isSimpleElderlyMode ? 'Simple mode disabled' : 'Simple elderly mode enabled');
              }}
            ...
            >
              {isSimpleElderlyMode ? 'Enabled' : 'Enable'}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { id: 'normal', label: t('accessibility.contrastNormal'), desc: 'Standard colors' },
              { id: 'high', label: t('accessibility.contrastHigh'), desc: 'High visibility & sharp outlines' },
            ].map(item => (
              <button
                key={item.id}
                onClick={() => {
