...
export const PatientSettingsPage: React.FC = () => {
  ...
  const primaryCaregiver = caregivers[0] || {
    name: patient?.emergencyContact?.name || 'Priya Sharma',
    relationship: patient?.emergencyContact?.relation || 'Family',
    phone: patient?.emergencyContact?.phone || '+91 98765 43210'
  };

  return (
    <PatientLayout pageTitle={t('accessibility.title')}>
      <div className="space-y-6 max-w-2xl mx-auto font-sans">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-soft flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
          <div className="flex items-center gap-5">
            <img
              src={patient.photoUrl || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&auto=format&fit=crop&q=80'}
              alt={patient.name}
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-slate-500 font-medium">
                Age {patient.age || 65} • {patient.gender || 'Senior'} • {patient.location || 'Assam, India'}
              </p>
              <span className="inline-block mt-1 text-xs font-bold text-teal-800 bg-teal-100 px-3 py-0.5 rounded-full">
                {patient.cognitiveBaseline || 'Mild Cognitive Impairment (Early Support)'}
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-soft divide-y divide-slate-100 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">English • অসমীয়া • বাংলা • हिन्दी</div>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="font-extrabold text-slate-900 text-base">{t('accessibility.title')}</div>
                <div className="text-xs text-slate-500">{t('accessibility.subtitle')}</div>
              </div>
            </div>
            <ChevronRight className="text-slate-400" size={20} />
          </div>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="font-extrabold text-slate-900 text-base">{t('assessment.title')}</div>
                <div className="text-xs text-slate-500">{t('assessment.subtitle')}</div>
              </div>
            </div>
            <ChevronRight className="text-slate-400" size={20} />
          </div>
