...
export const RoutineTrackerPage: React.FC = () => {
  const { routineItems, toggleRoutineCompleted, t, speakText } = useApp();

  const completedCount = routineItems.filter(r => r.isCompleted).length;

  return (
    <PatientLayout pageTitle={t('routine.title')}>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-soft flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <SpeechSpeaker textToSpeak={`${t('routine.title')}: ${completedCount} of ${routineItems.length} ${t('routine.completed')}.`} />
            </div>
          ...
          <div className="bg-indigo-50 border border-indigo-200 px-5 py-2.5 rounded-2xl text-center">
            <div className="text-xs text-indigo-800 font-bold uppercase">{t('caregiver.routineCompletion')}</div>
            <div className="text-2xl font-black text-indigo-950">
              {completedCount} / {routineItems.length} {t('routine.completed')}
            </div>
          </div>
        </div>
