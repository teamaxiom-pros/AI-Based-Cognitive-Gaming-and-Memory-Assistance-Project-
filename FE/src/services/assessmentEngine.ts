...
export function scoreAssessmentResponses(
  ...
  if (domainScores['executive_function'] && !domainScores['sequencing']) {
    domainScores['sequencing'] = { ...domainScores['executive_function'], domain: 'sequencing' };
  }
  if (domainScores['memory'] && !domainScores['recall']) {
    domainScores['recall'] = { ...domainScores['memory'], domain: 'recall' };
  }

  // Calculate overall average score from actively evaluated domains
  const activeEvaluatedDomains = Object.values(domainScores).filter(d => d.taskCount > 0);
  const overallScore = activeEvaluatedDomains.length > 0
    ? Math.round(activeEvaluatedDomains.reduce((acc, d) => acc + d.score, 0) / activeEvaluatedDomains.length)
    : 75;

  // Determine personalized game recommendations based strictly on lowest performance domains
  const sortedDomains = (activeEvaluatedDomains.length > 0 ? activeEvaluatedDomains : Object.values(domainScores))
...
export function saveAssessmentSession(session: AssessmentSession): void {
  try {
    ...
    const result: AssessmentResult = {
      ...
      completedAt: session.endTime ? new Date(session.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
  ...
  try {
    ...
    if (sessions.length > 0) {
      ...
      return {
        ...
        completedAt: latest.endTime ? new Date(latest.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
